...
| config.database.url | string | `"file:main?mode=memory&cache=shared"` | Database connection URL |
| config.envSecret | string | `""` | Name of secret containing environment variables |
...
| gateway.className | string | `"envoy-gateway"` | Gateway class name |
| gateway.enabled | bool | `true` | Enable API gateway |
| gateway.externalHostname | string | `"clearnode.example.com"` | External hostname for the gateway |
| gateway.ipAddressName | string | `""` | GKE static IP address name (GKE only) |
| gateway.tlsClusterIssuer | string | `"zerossl-prod"` | TLS cluster issuer |
